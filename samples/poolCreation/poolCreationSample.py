...
def main():
    ...
    initialBalancesSorted = [
        pool["tokens"][token]["initialBalance"] for token in tokensSorted
    ]
    ...
    print()
    creationHash = None
    if "poolId" not in pool.keys():
        txHash = bal.balCreatePoolInFactory(
            pool, gasFactor, gasSpeed, gasPriceGweiOverride=gasPriceGweiOverride
        )
        if not txHash:
            quit()
    ...
    try:
        time.sleep(5)
        # sleep ensure that the RPC knows the new pool exists
        txHash = bal.balJoinPoolInit(
            pool, poolId, gasPriceGweiOverride=gasPriceGweiOverride
        )
    except Exception as e:
        print("Joining pool failed!")
        print(
            "Depending on the pool type, this could be due to you not being the pool owner"
        )
    ...
    if creationHash is not None:
        command = bal.balGeneratePoolCreationArguments(
            "0x" + poolId, creationHash=creationHash
        )
    else:
